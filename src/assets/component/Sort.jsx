...
const Sort = () => {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:5001/RoomsPage?filter=${filter}&sort=${sort}&search=${search}&page=${currentpage}&size=${itemperpage}`)
            .then(res => res.json())
            .then(data => {
                setdata(data);
    ...
    }, [filter, sort, search, currentpage, itemperpage]);

    useEffect(() => {
        fetch('http://localhost:5001/coffeepage')
            .then(res => res.json())
            .then(data => {
                console.log(data)
                setcount(data.count)
            ...
            <div>
                <div className='flex flex-col md:flex-row justify-center items-center gap-5 '>
                    <div>
                        ...
                        >
                            <option value=''>Filter By flower</option>
                ...
                <div className='grid grid-cols-1 gap-8 mt-8 xl:mt-16 md:grid-cols-2 lg:grid-cols-3 '>
                    {data.map(item => (
                        <Sortdetails item={item}></Sortdetails>
                    ))}
                </div>
            </div>
